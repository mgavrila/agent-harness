 ...
 * process serving the `default` client, auditing every call as `svc-host`, or pointing a
 * registry lookup at a live endpoint, every one of them silently. Unset keeps the default;
 * set-but-empty fails startup naming the variable.
 */
