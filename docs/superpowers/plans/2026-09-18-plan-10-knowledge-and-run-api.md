...
- Produces: `knowledgeTools: AnyToolDef[]` holding `knowledge_search` (`read`) and `knowledge_sync` (`admin`), added to `kernelTools(packs)`.
...
| 5.7, `knowledge_sync` `admin`, walks `clients/<name>/knowledge/**/*.md`, frontmatter `title` / `min_level` / `principals`, upserts by `sha256`, tombstones removed files | Task 4 (engine) + Task 6 (tool) |
