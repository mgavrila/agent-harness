...
- **Two migrations in this plan: Task 3's `client_secrets` and Task 8's drop of `playbooks.timezone`** (decision 16, the user's UTC-everywhere ruling). Both generated by plain `drizzle-kit generate`, and neither is ever `--custom` or hand-edited. **No other task touches `harness/db/src/domain/schema.ts` or `harness/db/drizzle/`.** Each of the two runs the generator **twice**, and the second run prints that there is nothing to migrate, which is the check that the committed SQL is what the schema says. `harness/db/src/testing.ts`'s truncation list gains the new table in Task 3, the same task that adds it, or every database-backed suite leaks rows between cases; Task 8 drops a column from an existing table and touches no truncation list.
...
- **Snapshots.** The **tool-surface snapshot** (`docs/architecture/tool-surface.json`) is byte-identical in **every task of this plan except Task 8, which re-records the tool surface once**: dropping `playbooks.timezone` (decision 16) drops the field from `playbooks_list`'s output schema, which is a tool description change the recorder has to pick up. No other task adds, removes or re-describes a tool, and the recorder's fixture document — `fixtureDocument()` in `@harness/config-api/testing`, used by `surfaceDeps()` in `record-surface.ts` — declares `surfaces: { memory: {}, http: {} }`, no `web` section and no `{ ref }`, so nothing else this plan adds to the schema reaches it. `git status --short docs/architecture` proves it everywhere but Task 8, where the same check has to show exactly `tool-surface.json` and nothing else. The **Compose snapshot** (`docs/architecture/compose-surface.yaml`) changes in exactly **one** task, **Task 3**, which is the only task that edits `harness/compose/docker-compose.yml`; Task 3 and Task 8 are now the two tasks that run `pnpm surface:record`, and Task 8's run must leave `compose-surface.yaml` untouched. Every other task leaves both files byte-identical, and a task that finds otherwise has broken something it did not mean to.
...
16. **The playbook schedule loses its timezone; the scheduler evaluates every cron expression in UTC.** The kernel's one remaining timezone was the client document's `timezone` field on a playbook, threaded through `@harness/config-api`'s `PlaybookShape`, `@harness/core-tools`'s `PlaybookSummary` and `playbooks_list`'s output, the `playbooks.timezone` column, and the host's `nextRunAfter` into `croner`. It is removed rather than defaulted: the field, the column and the output property are all deleted, and `nextRunAfter` calls `croner` with `timezone: 'UTC'` as a constant the scheduler owns, never a value a document supplies. Task 8 carries it because it is the boundary task with no seam of its own, exactly as it already carries the deepagents repair — which is also why it costs this plan a second migration and a second tool-surface re-record, both otherwise exceptions to Global Constraints. User ruling 2026-09-22: UTC everywhere; the two-migration and one-re-record exceptions follow from it.
...
`.dependency-cruiser.cjs` (one alternation gains `deploy`); `harness/core-tools/src/kernel-vocabulary.test.ts` (the comment and one case); `harness/core-tools/src/app/record-surface.ts` (export `SOURCE_ROOTS`) + `app/surface.test.ts` (one case); `scripts/src/domain/boundaries.test.ts` (**new**); `runtimes/deepagents/src/domain/run.test.ts` (one case's abort, which is this plan's one unrelated repair). **By amendment (decision 16), UTC everywhere:** `harness/config-api/src/playbooks.ts` + `playbooks.test.ts`; `harness/core-tools/src/domain/playbooks/types.ts`, `repository.ts` + `repository.test.ts`; `harness/core-tools/src/tools/playbooks.ts` + `playbooks.test.ts`; `harness/db/src/domain/schema.ts` + `schema.test.ts`, `harness/db/drizzle/0016_*.sql` + `meta/` (**generated**); `harness/host/src/domain/playbooks/schema.ts` + `schema.test.ts`, `repository.ts` + `repository.test.ts`, `scheduler.test.ts`, `preflight.test.ts`; `clients/fixture/client.yaml`; `docs/runbook.md`; `docs/architecture/tool-surface.json` (**re-recorded**).
...
**By amendment, 2026-09-22 (decision 16):** it also carries the user's UTC-everywhere ruling — nothing timezone-related may remain in the kernel. The playbook schedule's IANA `timezone` field was the kernel's only one, and this task, still the one with no seam of its own, is where it comes out: the field, the column and the tool output property are deleted, and the scheduler evaluates every cron expression in UTC by passing `timezone: 'UTC'` to `croner` as a constant rather than a value a document supplies.
...
- Modify: `harness/config-api/src/playbooks.ts` (drop `validTimezone` and the `timezone` field)
- Modify: `harness/config-api/src/playbooks.test.ts` (drop the timezone cases; add the unknown-key case)
- Modify: `harness/core-tools/src/domain/playbooks/types.ts` (`PlaybookSummary` drops `timezone`)
- Modify: `harness/core-tools/src/domain/playbooks/repository.ts` (`summarisePlaybook` drops `timezone`)
- Modify: `harness/core-tools/src/domain/playbooks/repository.test.ts` (drop the `timezone` expectation)
- Modify: `harness/core-tools/src/tools/playbooks.ts` (`SummaryShape` and the description drop `timezone`)
- Modify: `harness/core-tools/src/tools/playbooks.test.ts` (assert `timezone` is gone from the tool's output)
- Modify: `harness/db/src/domain/schema.ts` (drop the `playbooks.timezone` column — the plan's second migration)
- Modify: `harness/db/src/domain/schema.test.ts` (drop the `timezone` expectation)
- Generate: `harness/db/drizzle/0016_*.sql` + `meta/`
- Modify: `harness/host/src/domain/playbooks/schema.ts` (`nextRunAfter` drops its `timezone` parameter; `croner` is called with `timezone: 'UTC'` as a constant)
- Modify: `harness/host/src/domain/playbooks/schema.test.ts` (proves the schedule evaluates in UTC regardless of the process's own zone)
- Modify: `harness/host/src/domain/playbooks/repository.ts` (both `nextRunAfter` call sites; the `timezone` column in `syncPlaybooks`'s upsert)
- Modify: `harness/host/src/domain/playbooks/repository.test.ts`, `scheduler.test.ts`, `preflight.test.ts` (drop `timezone` from every `PlaybookDefinition`/`PlaybookRow` fixture)
- Modify: `clients/fixture/client.yaml` (drop the `timezone` line)
- Modify: `docs/runbook.md` (the Playbooks section's timezone sentences)
- Re-record: `docs/architecture/tool-surface.json` (`playbooks_list`'s output schema loses a field)
...
- Removes: `timezone` from `PlaybookDefinition` (`@harness/config-api`), `PlaybookSummary` and the `playbooks_list` output shape (`@harness/core-tools`), the `playbooks` table, and `nextRunAfter`'s signature (`(schedule, timezone, from, where?)` becomes `(schedule, from, where?)`).
...
- [ ] **Step 7: Write the failing UTC tests**

Three failing tests, one per package, each proving the shape this step group is about to build. **This is by amendment (decision 16)**: the user ruled that nothing timezone-related may remain in the kernel, and the playbook schedule's IANA `timezone` field is the only timezone left in it.

In `harness/config-api/src/playbooks.test.ts`, drop `timezone` from the `nightly` fixture:
...
Replace the `'defaults the timezone to UTC and accepts an empty file'` case with one that only proves the empty-file half — the default is gone along with the field:
...
and add a case beside it proving the shape refuses `timezone` now that it is not a field of its own — `PlaybookShape` is already `.strict()`, so an unknown key is refused without any new code, which is exactly what this case is written to prove before Step 9 removes anything:
...
In the `'refuses a bad schedule, a bad timezone, a user principal, a duplicate name and an unknown key'` case, drop the bad-timezone assertion and its clause from the title, which becomes `'refuses a bad schedule, a user principal, a duplicate name and an unknown key'`:
...
(the line above it, `expect(() => parsePlaybooksFile({ playbooks: [{ ...nightly, timezone: 'Nowhere/City' }] }))…`, is deleted outright — the case just above now covers an unrecognised `timezone` value the same way it covers every other unknown key).

In `harness/core-tools/src/tools/playbooks.test.ts`, add a case beside the listing one:
...
In `harness/host/src/domain/playbooks/schema.test.ts`, replace the whole file. The new case drives a real cron tick under a process zone that is **not** UTC and proves the firing still lands on the UTC instant — the one proof that actually pins `timezone: 'UTC'` as a constant the scheduler passes, because croner's default when no `timezone` option is given is the *process's own* zone, not UTC, and a case that only ever ran under a UTC test runner could pass by accident forever:
...
Run:
```bash
pnpm --filter @harness/config-api exec vitest run src/playbooks.test.ts
pnpm --filter @harness/core-tools exec vitest run src/tools/playbooks.test.ts
pnpm --filter @harness/host exec vitest run src/domain/playbooks/schema.test.ts
```
Expected: FAIL, all three. `PlaybookShape` still declares `timezone` and takes both the value the new case sends and the unknown-key case's `America/New_York`, so neither config-api case sees a refusal. `SummaryShape` and `summarisePlaybook` still carry `timezone`, so the core-tools case finds the property it asserted is gone. `nextRunAfter` still takes `(schedule, timezone, from, where?)`; the two-argument call above passes `from` where `timezone` is expected and leaves the real `from` `undefined`, so the computed instant does not match either expectation.

- [ ] **Step 9: Remove the timezone field everywhere**

In `harness/config-api/src/playbooks.ts`, delete `validTimezone` (the whole function) and the `timezone` field of `PlaybookShape`:
...
The `Cron` import stays — `validSchedule` still uses it.

In `harness/core-tools/src/domain/playbooks/types.ts`, drop `timezone: string;` from `PlaybookSummary`.

In `harness/core-tools/src/domain/playbooks/repository.ts`, drop `timezone: row.timezone,` from `summarisePlaybook`.

In `harness/core-tools/src/tools/playbooks.ts`, drop `timezone: z.string(),` from `SummaryShape` and rewrite the tool's description:
...
In `harness/host/src/domain/playbooks/schema.ts`, `nextRunAfter` drops its `timezone` parameter and calls `croner` with `timezone: 'UTC'` as a constant, not data:
...
In `harness/host/src/domain/playbooks/repository.ts`, drop `timezone: def.timezone,` from `syncPlaybooks`'s `values` object, and update both `nextRunAfter` call sites:

```ts
        nextRunAt: def.enabled ? nextRunAfter(def.schedule, opts.now, where(def.name)) : null,
```

and, in `claimDuePlaybooks`:

```ts
          nextRunAt: nextRunAfter(playbook.schedule, opts.now, `playbook "${playbook.name}"`),
```

Then the fixtures that fed the field being removed: drop the `timezone: 'UTC',` line from `harness/core-tools/src/domain/playbooks/repository.test.ts`'s `summarisePlaybook` expectation; from the `nightly: PlaybookDefinition` fixture in `harness/host/src/domain/playbooks/repository.test.ts` and `harness/host/src/domain/playbooks/scheduler.test.ts`'s `NIGHTLY`; and from the `row()` builder in `harness/host/src/domain/playbooks/preflight.test.ts`. In that same file, the fixture-client case's assertion loses the field too:

```ts
      expect(playbook).toMatchObject({ principal: 'svc-playbooks', deliver: 'none' });
```

In `harness/db/src/domain/schema.test.ts`, drop `timezone: 'UTC', ` from the `toMatchObject` call that checks a freshly inserted playbook row's defaults.

In `clients/fixture/client.yaml`, drop the `timezone: America/New_York` line from the `knowledge-refresh` playbook.

In `docs/runbook.md`'s Playbooks section, drop the timezone field from the section's own list, drop the "timezone is checked at parse time too" sentence, restate the scheduler's tick as always-UTC, and restate the demo playbooks' times in UTC rather than a zone:
...
the host at open, naming the field.
```

(the paragraph's last two sentences, about a removed playbook being disabled rather than deleted and `next_run_at` being recomputed at every start, are unchanged and follow immediately after).

**One thing this step deliberately does not touch**: `harness/db/src/domain/migration-0012.test.ts` applies only migration `0012`'s own statements to a scratch database, seeded and torn down inside that one file, and asserts what that historical migration produced — `timezone` was still a column when `0012` ran, so its `SELECT timezone, …` and its `{ timezone: 'UTC', … }` expectation both stay exactly as they are. The column comes out in Step 10's migration, `0016`, which this test never applies.

- [ ] **Step 10: Drop the column — the second migration of the plan**

In `harness/db/src/domain/schema.ts`, drop the `timezone` column from `playbooks` and fold what it said into the `schedule` comment:
...
(the line `timezone: text('timezone').notNull().default('UTC'),` is deleted; nothing else in the table changes).

Then generate, from `harness/db/`:

```bash
pnpm --filter @harness/db generate
```

Expected: one new file, `harness/db/drizzle/0016_<two words drizzle-kit chose>.sql`, holding exactly:

```sql
ALTER TABLE "playbooks" DROP COLUMN "timezone";
```

plus an entry in `drizzle/meta/_journal.json` and a new `meta/0016_snapshot.json`. **Read the SQL before going on**: it must alter nothing but this one column. Then run it again:

```bash
pnpm --filter @harness/db generate
```

Expected: `No schema changes, nothing to migrate`. **Do not hand-write or hand-edit either file, and never `--custom`.** `harness/db/src/testing.ts`'s truncation list needs no change — a column is dropped, not a table added or removed.

- [ ] **Step 11: Re-record the tool surface**

`playbooks_list`'s output schema just lost `timezone` (Step 9), which is a tool description change, and the tool surface snapshot has to say so — the one exception to Global Constraints' "byte-identical in every task" rule.
...
Expected: `docs/architecture/tool-surface.json` changed and **`docs/architecture/compose-surface.yaml` did not** — this task touches no compose file, so the recorder's other half has nothing to pick up. If `compose-surface.yaml` moved, stop: something here reached a file no task in this plan is meant to touch. The diff itself is the `timezone` property and its place in `required` disappearing from `playbooks_list`'s output schema; nothing else in the snapshot moves.

- [ ] **Step 12: Run the suites green**
...
pnpm arch
```
Expected: PASS — three boundary cases, the vocabulary suite with its new case and its empty allowlist, the surface suite with its new case, config-api's playbook shape refusing an unknown `timezone` key, core-tools' tool output carrying no `timezone`, the host's `schema.test.ts` proving a cron tick evaluates in UTC under a non-UTC process zone plus the rest of the host's playbook suites with their fixtures trimmed, `schema.test.ts`'s (`@harness/db`) row with no `timezone` column, the deepagents run suite, and zero architecture violations.
...
Run the gate command from Task 2 Step 14.
Expected: all green, at most 25 lint warnings. `git status --short docs/architecture` shows **only** `tool-surface.json` — Step 11 re-recorded it, and `compose-surface.yaml` does not move.

- [ ] **Step 14: Commit**

Three commits, because they are three changes and a reader bisecting any one of them should not land on either of the other two:
...
git add harness/config-api/src harness/core-tools/src harness/db/src harness/db/drizzle harness/host/src clients/fixture/client.yaml docs/runbook.md docs/architecture/tool-surface.json
git commit -m "refactor(playbooks): schedule in UTC and drop the timezone field"
```
...
**2026-09-22, after the user's ruling that nothing timezone-related may remain in the kernel.** The playbook schedule's IANA zone was the kernel's only timezone, and Task 8 — the boundary task with no seam of its own, already carrying the deepagents repair — is where it comes out, as new Steps 7–11 inserted before the task's original Step 7 (renumbered 12): `harness/config-api/src/playbooks.ts` (`validTimezone`, the `timezone` field and its `.refine`) and `playbooks.test.ts`; `harness/core-tools/src/domain/playbooks/types.ts` (`PlaybookSummary.timezone`), `repository.ts` and `repository.test.ts`; `harness/core-tools/src/tools/playbooks.ts` (the output schema's `timezone` field and the description sentence) and `playbooks.test.ts`; `harness/db/src/domain/schema.ts` (the `timezone` column, dropped by the plan's second migration, `0016`) and `schema.test.ts`; `harness/db/drizzle/0016_*.sql` + `meta/` (**generated**); `harness/host/src/domain/playbooks/schema.ts` (`nextRunAfter` drops its `timezone` parameter and calls `croner` with `timezone: 'UTC'` as a constant) and `schema.test.ts` (rewritten to drive a cron tick under a non-UTC process zone); `repository.ts` (both `nextRunAfter` call sites) and `repository.test.ts`; `scheduler.test.ts`; `preflight.test.ts`; `clients/fixture/client.yaml`; `docs/runbook.md`'s Playbooks section; and `docs/architecture/tool-surface.json`, re-recorded once because `playbooks_list`'s output schema lost a field — the plan's one exception, alongside Task 3's compose re-record, to Global Constraints' "byte-identical in every task" snapshot rule. Decision 16 records the ruling and why the two-migration and one-re-record exceptions follow from it; sections 6 and 7 above are updated for the third commit this adds to Task 8.
