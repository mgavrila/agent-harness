 ...
 * wrapper in `tools/aliases.ts` looks this pack up by name to work out which document kinds
 * belong to somebody else, and a name that had drifted from the declared one would not fail at
 * the edit — it would fail at startup, as a `ConfigError` reading `no pack named "…" is loaded`,
 * in whatever deployment happened to load this pack first.
 *
