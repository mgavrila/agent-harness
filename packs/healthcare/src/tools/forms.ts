 ...
 * The four form tools.
 *
 * Built at module load rather than per server, because none of them closes over the dependency
 * bag: each handler is given the live one as `d`, and the writes that used to call core-tools'
 * free functions — `writeOutFile`, `stageRelease` — reach them through `d.kernel`, because a
 * pack has no file-store handle of its own and the kernel is what keeps the out tree
 * client-scoped.
 */
